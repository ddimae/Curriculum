...
<html>
   <head>
      ...
      <title>Update DisciplineCurriculum</title>
   </head>
   <body>
  <jsp:include page="_header.jsp"></jsp:include>
      <jsp:include page="_menu.jsp"></jsp:include>
      <h1>Update Discipline Curriculum</h1>
 
      <p style="color: red;">${errorString}</p>
 
      <c:if test="${not empty DisciplineCurriculum}">
      <div class="form-cu">
         <form method="POST" action="${pageContext.request.contextPath}/editDisciplineCurriculum" enctype="multipart/form-data">
         ...
         <input type="hidden" name="file_url1" value="${DisciplineCurriculum.file_url}" />
            	Discipline
            	<select name="dis_id">
            	<option value="${DisciplineCurriculum.discipline.id}" selected>${DisciplineCurriculum.discipline.name}</option> 
            	...
            	Curriculum
            	<select name="cur_id">
            	<option value="${DisciplineCurriculum.curriculum.id}" selected>${DisciplineCurriculum.curriculum.name}</option> 
