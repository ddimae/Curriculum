...
@WebServlet(urlPatterns = {"/CurriculumList"})
public class CurriculumListServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    public CurriculumListServlet() {
        super();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                list1 = MyUtils.listCurriculums(conn);
                list = MyUtils.listCurriculumsPage(conn, (page - 1) * recordsPerPage, recordsPerPage);
